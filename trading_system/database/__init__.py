...
class Database:
    ...
    def browse_table(
        ...
        *,
        ...
        """Read-only table browser for dashboard. Allowlisted tables only."""
        allowed = {
            "trades",
            "rejected_signals",
            "insights",
        ...
        with self.connection() as conn:
            ...
            ob = order_by if order_by in cols else (cols[0] if cols else "rowid")
            direction = "ASC" if str(order_dir).upper() == "ASC" else "DESC"
            # Identifiers already validated against allowlist / pragma columns
            rows = conn.execute(
                f'SELECT * FROM {table} ORDER BY "{ob}" {direction} LIMIT ? OFFSET ?',
                (limit, offset),
            ).fetchall()
    ...
    def list_tables(self) -> list[dict[str, Any]]:
        ...
        with self.connection() as conn:
            for name in allowed:
                ...
                out.append({"name": name, "rows": int(n)})
        return out
